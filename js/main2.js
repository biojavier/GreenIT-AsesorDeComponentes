var controller = (function (jsonDB) {
	...
	searchItem = function(cantItems, vectorDeItems, usoSeleccionado, nombreLocalStorage){		
		var i=0;
		encuentra=false;					
		...
		*/
		...
		if(ndd == "Principiante"){
			...
			/*while(!encuentra && i<parseInt(jsonDB.producto.cantidadProcesadores)){
				sugerido = jsonDB.producto.procesadores[i].sugerido;
				if(sugerido == usoSeleccionado){
					localStorage.setItem("procesador",jsonDB.producto.procesadores[i].nombre);					
					encuentra=true;
					costo = costo + parseInt(jsonDB.producto.procesadores[i].precio);
				}else{
					i++;
				}
			}*/
			costo += searchItem(parseInt(jsonDB.producto.cantidadProcesadores),jsonDB.producto.procesadores, usoSeleccionado, "procesador");
			/*i=0;
			encuentra=false;
			while(!encuentra && i<parseInt(jsonDB.producto.cantidadDiscos)){
				sugerido = jsonDB.producto.discos[i].sugerido;
				if(sugerido == usoSeleccionado){
					localStorage.setItem("discoRigido",jsonDB.producto.discos[i].nombre);					
					encuentra=true;
					costo = costo + parseInt(jsonDB.producto.discos[i].precio);
				}else{
					i++;
				}
			}*/
			costo += searchItem(parseInt(jsonDB.producto.cantidadDiscos),jsonDB.producto.discos, usoSeleccionado, "discoRigido");
			i=0;
			encuentra=false;
			while(!encuentra && i<parseInt(jsonDB.producto.cantidadFuentes)){
				sugerido = jsonDB.producto.fuentes[i].sugerido;
				if(sugerido == usoSeleccionado){
					localStorage.setItem("fuente",jsonDB.producto.fuentes[i].nombre);					
					encuentra=true;
					costo = costo + parseInt(jsonDB.producto.fuentes[i].precio);
				}else{
					i++;
				}
			}
			...
			while(!encuentra && i<parseInt(jsonDB.producto.cantidadMemoriasRam)){
				sugerido = jsonDB.producto.memoriasRam[i].sugerido;
				if(sugerido == usoSeleccionado){
					localStorage.setItem("memoriaRam",jsonDB.producto.memoriasRam[i].nombre);					
					encuentra=true;
					costo = costo + parseInt(jsonDB.producto.memoriasRam[i].precio);
				}else{
					i++;
				}
			}
			...
			while(!encuentra && i<parseInt(jsonDB.producto.cantidadPlacasMadre)){
				sugerido = jsonDB.producto.placasMadre[i].sugerido;
				if(sugerido == usoSeleccionado){
					localStorage.setItem("placaMadre",jsonDB.producto.placasMadre[i].nombre);					
					encuentra=true;
					costo = costo + parseInt(jsonDB.producto.placasMadre[i].precio);
				}else{
					i++;
				}
			}
			...
			while(!encuentra && i<parseInt(jsonDB.producto.cantidadPlacasVideo)){
				sugerido = jsonDB.producto.placasVideo[i].sugerido;
				if(sugerido == usoSeleccionado){
					localStorage.setItem("placaVideo",jsonDB.producto.placasVideo[i].nombre);					
					encuentra=true;
					costo = costo + parseInt(jsonDB.producto.placasVideo[i].precio);
				}else{
					i++;
				}
			}	
			if(!encuentra){
				localStorage.setItem("placaVideo","ninguna");					
			}
